...
- 출처: Hacker News
- 원본 링크: https://www.agentconnect.md/blog/grep-beat-lsp-harness/
- 발행: 2026-09-04T03:40:26+00:00
- 접근상태: 확인 완료

---

Grep beats LSP? Why coding agents ignore your fancier tools — AgentConnect Blog Docs Blog Star  1.3k Get started All posts 
 ...
 Engineering 9 min read Grep beats LSP? Why coding agents ignore your fancier tools I compared grep with LSP-backed semantic navigation across code-finding and editing tasks. The results show why a tool's LLM-friendliness may matter as much as the capability behind it.
 PX Pengcheng Xu Engineering · 2026-08-12 Copy link Why would a coding agent ignore a retrieval interface that returns more precise results?
 ...
 Structure: some tasks need textual completeness, which the tested semantic-navigation methods do not provide. 
 Distribution: the model may have more practice with familiar tools and result shapes. 
 ...
 Test real tasks at equal accuracy. Do not celebrate lower token use if success also fell. 
 Measure whether the agent calls it. Availability is not adoption. 
 Return enough context for the next decision. A result like path:line:content may work better than a bare location object. 
 Keep a native fallback. Semantic and lexical search solve different problems. 
 Route by the task and the codebase. A noisy repository may benefit from semantic navigation. A text-wide search may still need grep . 
 Reinforce the new trajectory when it matters. A prompt can introduce a tool, but it may not create a reliable policy for using it. 
 ...
 Star AgentConnect on GitHub agentconnect-md/agentconnect 
 Get started with AgentConnect docs.agentconnect.md 
 ...
 # agent-harness # tool-use # ACP # engineering On this page Comparing two code retrieval interfaces Tool interfaces change agent behavior Why lexical search had an advantage The harness is part of the system Practical guidance for adding tools Conclusion Keep reading All posts → Product AgentConnect — the open-source, multi-agent alternative to Claude Tag 
 
 
 
 
 Product Why we built AgentConnect Put an agent in your first channel. Open source and self-hosted — run AgentConnect on your own infrastructure, bring any agent, and keep full control. No lock-in.
 Get started Star  1.3k Agent Connect Docs Blog Terms Privacy