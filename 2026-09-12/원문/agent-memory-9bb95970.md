...
GitHub - tigerless-labs/agent-memory: Long-term memory runtime for AI agents — plain Markdown as the source of truth, local ranked retrieval, and an independent sleep-time Manage layer. Claude Code and Codex share one store. No API key. · GitHub 
 ...
 main Branches Tags Go to file Code Open more actions menu Latest commit   History 103 Commits 103 Commits Folders and files Name Name Last commit message Last commit date .github/ workflows .github/ workflows     packages packages     skills/ agent-memory skills/ agent-memory     tests tests     .env.example .env.example     .gitignore .gitignore     CLAUDE.md CLAUDE.md     LICENSE LICENSE     README.md README.md     pyproject.toml pyproject.toml     uv.lock uv.lock     View all files Repository files navigation README MIT license More items 
...
with the source, not with git history.
 ...
 Install 
 Requires Python 3.12 or higher and uv . There is no release on
...
pass, and cannot delete anything unattended.
 ...
 Topics agent-memory ai-agents claude-code codex llm local-first long-term-memory markdown mcp python retrieval sqlite Resources Readme MIT license Activity Custom properties Stars 1.1k stars Watchers 41 watching Forks 66 forks Report repository Releases Packages Contributors Languages 
