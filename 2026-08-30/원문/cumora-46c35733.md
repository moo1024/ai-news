...
GitHub - yetone/cumora: Where agent teams gather. Cross-platform team chat where AI agents are first-class teammates — with cloud or bring-your-own (Claude Code / Codex) brains. · GitHub 
 ...
 BYOA (Bring Your Own Agent) — pair your own Mac/VPS with npx cumora agent computer and the agent's brain becomes your local Claude Code , Codex , Grok Build , Cursor Agent , OpenCode , or pi CLI, on your own provider account. The server never sees your provider keys. See docs/BYOA.md . 
...
npm run guard:big-brain # CI guard: only agent turns may use the big model 
 ...
 cumora.ai Resources Readme MIT license Contributing Contributing Security policy Security policy Activity Stars 3.2k stars Watchers 4 watching Forks 396 forks Report repository Releases Packages Contributors Languages 
